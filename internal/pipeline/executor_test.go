...
func TestResume_WithPriorState(t *testing.T) {
	t.Parallel()

	workflow := &Workflow{
		Name: "test-resume",
		Steps: []Step{
			{ID: "step1", Prompt: "already done", Agent: "claude"},
			{ID: "step2", Prompt: "resume here", Agent: "codex", DependsOn: []string{"step1"}},
		},
	}
	...
	priorState := &ExecutionState{
		...
		Steps: map[string]StepResult{
			"step1": {StepID: "step1", Status: StatusCompleted, Output: "done"},
		},
		Variables: map[string]interface{}{
			"myvar": "preserved",
		},
	...
	state, err := e.Resume(context.Background(), workflow, priorState, nil)
	if err != nil {
		t.Fatalf("Resume() error = %v", err)
	}
	if state.RunID != "prior-run-123" {
		t.Errorf("RunID = %q, want preserved prior RunID", state.RunID)
	}
	if state.Variables["myvar"] != "preserved" {
		t.Error("Variables should be preserved from prior state")
	}
	if state.Status != StatusCompleted {
...
func TestResume_FillsDefaults(t *testing.T) {
	...
	workflow := &Workflow{
		Name: "fill-defaults",
		Steps: []Step{
			{ID: "s1", Prompt: "test", Agent: "claude"},
		},
	}

	priorState := &ExecutionState{
		// Minimal state - all fields empty
		Steps:     nil,
		Variables: nil,
		RunID:     "",
	}

	cfg := DefaultExecutorConfig("my-session")
	cfg.DryRun = true
	cfg.WorkflowFile = "/path/to/workflow.yaml"
	cfg.RunID = "cfg-run-id"
	e := NewExecutor(cfg)
	...
	if state.RunID != "cfg-run-id" {
		t.Errorf("RunID = %q, want %q (from config)", state.RunID, "cfg-run-id")
	}
	if state.Session != "my-session" {
		t.Errorf("Session = %q, want %q", state.Session, "my-session")
	}
	if state.WorkflowFile != "/path/to/workflow.yaml" {
		t.Errorf("WorkflowFile = %q, want from config", state.WorkflowFile)
	}
	if state.WorkflowID != "fill-defaults" {
		t.Errorf("WorkflowID = %q, want %q", state.WorkflowID, "fill-defaults")
	}
}

func TestResume_WithDependencyError(t *testing.T) {
	t.Parallel()

	workflow := &Workflow{
		Name: "bad-deps",
		Steps: []Step{
			{ID: "s1", Prompt: "test", Agent: "claude", DependsOn: []string{"nonexistent"}},
		},
	}
	...
	state, err := e.Resume(context.Background(), workflow, priorState, nil)
	if err == nil {
		t.Fatal("Resume() should error on dependency validation failure")
	}
	if state.Status != StatusFailed {
		t.Errorf("Status = %v, want failed", state.Status)
	}
	if len(state.Errors) == 0 {
		t.Error("Errors should contain dependency error")
	}
}
