...
func DefaultAgentTemplates() AgentConfig {
	return AgentConfig{
		...
		Codex:    `{{if .SystemPromptFile}}CODEX_SYSTEM_PROMPT="$(cat {{shellQuote .SystemPromptFile}})" {{end}}codex --dangerously-bypass-approvals-and-sandbox -m {{shellQuote (.Model | default "gpt-5.2-codex")}} -c model_reasoning_effort="xhigh" -c model_reasoning_summary_format=experimental --enable web_search=live`,
